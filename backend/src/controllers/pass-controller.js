...
exports.generatePass = async (req, res) => {
    try {
        ...
        if (appointment.status !== "approved") {
            return res.status(400).json({ success: false, message: "Appointment not approved" });
        }
        ...
        if (visitDate < today) {
            return res.status(400).json({ success: false, message: "Visit date has already passed" });
        }

        const existingPass = await Pass.findOne({ appointmentId });
        if (existingPass) {
            return res.status(400).json({ success: false, message: "Pass already generated" });
        }

        const qrData = `PASS-${appointmentId}`;
...
Thank You.`
        ...
        res.status(201).json({ success: true, message: "Pass generated", data: pass });
    } catch (error) {
...
exports.generatePdf = async (req, res) => {
    try {
        ...
        doc.moveDown();
        doc.fontSize(12).text(`Visitor: ${pass.appointmentId.visitorId.name}`);
        doc.text(`Host: ${pass.appointmentId.hostId.name}`);
        doc.text(`Purpose: ${pass.appointmentId.purpose}`);
        doc.text(`Visit Date: ${pass.appointmentId.visitDate}`);
        doc.text(`Pass ID: ${pass._id}`);

        const base64Data = pass.qrImage.replace(/^data:image\/png;base64,/, "");
